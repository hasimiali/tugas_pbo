...
public class BouncingBallSimple extends JPanel {
	...
	int randomX = randomNum.nextInt(640);
	int randomY = randomNum.nextInt(480);
	private final int BALL_X_INITIAL_POSITION = randomX;
	private final int BALL_Y_INITIAL_POSITION = randomY;
	...
	private float ballSpeedY = (float) 0.75;
	
	// Jumlah refresh layar dalam satu detik misal 40 frame per second.
	...
	public BouncingBallSimple() {
		...
		ballSpeedY+=0.25;
		//friction
		if(ballSpeedX>0) ballSpeedX-=0.001;
		else if(ballSpeedX<0) ballSpeedX+=0.001;
		...
		ballSpeedY = (float) -(ballSpeedY+0.25);
		ballY = ballRadius;
